async function fetchAboutData() {
    try {
        const response = await fetch('http://176.124.210.146:3000/api/about')
        ...
        document.getElementById('image1').src = 'http://176.124.210.146:3000/' + aboutData?.image1?.url
        document.getElementById('image2').src = 'http://176.124.210.146:3000/' + aboutData?.image2?.url
        ...
        aboutData.cards.forEach(card => {
            console.log('http://176.124.210.146:3000/' + card.icon.url)
                ...
                <div class="relative">
                    <div class="bg-white rounded-2xl shadow-lg p-8 relative z-10 h-full">
                        <div class="w-12 h-12 bg-brand-100 rounded-2xl flex items-center justify-center mb-6">
                                <img src="${'http://176.124.210.146:3000/' + card?.icon?.url}" alt="${card?.title}-icon">
                        </div>
...
document.getElementById('ctaForm').addEventListener('submit', async function (e) {
    ...
    try {
        const response = await fetch('http://176.124.210.146:3000/api/forms', {
            method: 'POST',
            headers: {
